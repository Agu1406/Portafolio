...
// Event listeners
cargarEscudoBtn.addEventListener('click', () => {
    cargarEscudo()
        .then(mensaje => {
            ocultarLoader();
...
cargarTodoBtn.addEventListener('click', () => {
    // Usamos Promise.all para cargar todo a la vez
    Promise.all([cargarEscudo(), cargarJugadores()])
        .then(resultados => {
            ocultarLoader();
...
simularErrorBtn.addEventListener('click', () => {
    simularError()
        .then(mensaje => {
            ocultarLoader();
            mostrarExito(mensaje);
...
// Mensaje inicial
mostrarExito('Haz clic en los botones para cargar los datos usando promesas'); 