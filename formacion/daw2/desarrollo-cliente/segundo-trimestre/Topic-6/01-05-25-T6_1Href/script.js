...
// Guardo todos los enlaces existentes en una variable.
let enlaces = document.getElementsByTagName("a");

// Mediante posiciones obtengo el penultimo (dos posiciones menos)
let penultimoEnlace = enlaces[enlaces.length - 2];

// Imprimo su contenido usando el atributo ".href".
console.log("La dirección del penúltimo enlace es: " + penultimoEnlace.href);

let contador = 0;
for (let enlace of enlaces) {
    if (enlace.href.includes("/wiki/ISBN") || enlace.href.includes("/wiki/ISBN/")) {
        contador++;
    }
}

console.log("Hay " + contador + " enlaces que llevan a wiki/ISBN");